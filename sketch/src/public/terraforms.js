...
if ((charSet.push(originalChars), isOrigin))
  if (SEED > 9e3) for (let e of uni) charSet.push(makeSet(e));
  else charSet.push(makeSet(uni[Math.floor(SEED) % uni.length]));
else if (SEED > 9970) for (let e of uni) charSet.push(makeSet(e));
...
for (let e = 0; e < 32; e++) {
  ...
  for (let t = 0; t < 32; t++) {
    ...
    (grid[e][t] = r),
      ...
      (r.originalH = r.h),
      isDaydream &&
        ((r.style.webkitUserSelect = 'none'), (r.style.userSelect = 'none')),
      (r.onpointermove = (i) => {
        if (pointerDown && isDaydream) {
          for (let r = -brushSize; r < brushSize; r++)
            for (let i = -brushSize; i < brushSize; i++) {
              let a =
                grid[Math.min(Math.max(0, e + r), 31)][
                  Math.min(Math.max(0, t + i), 31)
                ];
...
setInterval(() => {
      ...
      0 == MODE
        ? r.h > 6 - RESOURCE &&
          (r.textContent =
            mainSet[
              Math.floor(
                0.25 * airship + (r.h + 0.5 * e + 0.1 * DIRECTION * t),
              ) % mainSet.length
            ])
        : (isDaydream || isTerraformed) &&
          (0 == r.h
            ? SEED < 8e3
              ? (r.textContent =
                  mainSet[
                    Math.floor(airship / 1e3 + 0.05 * e + 0.005 * t) %
                      mainSet.length
                  ])
              : (r.textContent =
                  mainSet[Math.floor(airship / 2 + 0.05 * e) % mainSet.length])
            : ((r.textContent =
                charSet[
                  Math.floor(airship / speedFac + e + r.h) % charSet.length
                ]),
              SEED > 5e3 &&
                Math.random() < 0.005 &&
                (r.style.fontSize = `${3 + (airship % 34)}px`))),
        ('j' != r.originalClass && 'j' != r.activeClass) ||
          (r.textContent = ' ');
    }
  airship += 1;
}, loopLength),
  ...
  document.addEventListener('keydown', (e) => {
    'e' == e.key
      ? (isEraser = !0)
      : 'q' == e.key
      ? (brushSize = (brushSize + 1) % 4)
      : 'a' == e.key && pointerShift++;
  }),
